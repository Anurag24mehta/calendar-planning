...
import {CalendarX2, Clock, Loader, VideoIcon, PlusCircle, XCircle} from "lucide-react";
...
export default function BookingFormPage({ params, searchParams}: {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`/api/booking`,{
                    ...
                    body: JSON.stringify({ username: params.username, eventUrl: params.eventUrl }),
                });
        ...
        fetchData();
    }, [params.username, params.eventUrl]);
    ...
    useEffect(() => {
        ...
        checkCondition()
    }, [searchParams.date, searchParams.time, bookingCheck]);

    const  onSubmit = async (formData: FormData) => {
        formData.username = params.username;
        formData.eventTypeId = data?.eventType[0].id as string;
        formData.eventDate  = searchParams.date;
        formData.fromTime = searchParams.time;
        formData.duration = data?.eventType[0].duration as number;
        formData.provider = data?.eventType[0].videoCallSoftware as string;
    ...
    return (
        <div>
            {data ? (
                <div className={"min-h-screen w-screen flex flex-col items-center justify-center"}>
                    <Card className={"max-w-[1280px] w-full mx-auto shadow-lg border"}>
                        <CardContent className={"p-5 md:grid md:grid-cols-[1fr,auto,1fr,auto,1fr,auto,1fr]"}>
                            <div>
                                <img src={data.image!} alt={"Profile Image"} className={"w-10 h-10 rounded-full"}/>
                                <p className={"text-sm font-muted text-muted-foreground mt-2"}>{data.name}</p>
                                <h1 className={"text-xl font-semibold mt-2"}>{data.eventType[0].title}</h1>
                                <p className={"text-sm font-muted text-muted-foreground mt-2"}>{data.eventType[0].description}</p>
                                <div className={"mt-5 flex flex-col gap-3"}>
                                    ...
                                    <p className={"flex items-center"}>
                                        ...
                                        <span
                                            className={"text-sm font-medium text-muted-foreground"}>{data.eventType[0].duration} minutes</span>
                                    </p>
                                    <p className={"flex items-center"}>
                                        <VideoIcon className={"size-5 mr-2 text-red-600"}/>
                                        <span
                                            className={"text-sm font-medium text-muted-foreground"}>{data.eventType[0].videoCallSoftware}</span>
                                    </p>
                                </div>
                            </div>
                            <Separator orientation={"vertical"} className={"f-full w-[2px] mx-2"}/>
                            <CalendarForm activity={data.availability}/>
                            <Separator orientation={"vertical"} className={"f-full w-[2px] mx-2"}/>
                            <TimeTablePage
                                selectedDate={selectedDate}
                                userName={params.username}
                                duration={data.eventType[0].duration}
                            ...
                            <form
                                ...
                                <div
                                    ...
                                    <p className={"text-muted-foreground mb-2"}>Enter  the detail of participants you want to add</p>
                                ...
                                <div className="flex items-center justify-between mt-4">
                                    ...
                                    <Button type="submit" className="ml-auto bg-muted hover:bg-red-300" variant={"outline"}>
                                        Book Meeting
                                    </Button>
                                </div>
                            </form>
                ...
                <Loader className={"animate-spin w-12 h-12 text-red-600 mx-auto min-h-screen"}/>
            )}
        </div>
    );
}
